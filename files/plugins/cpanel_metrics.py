...
def getSuspendedUsersCount90d():
    ...
    for user in all_users_list:
        if matchFilesLine(path, user, 'SUSPENDED=1', inverted=False) and not user in USERS_BLACKLIST:
            ...
            if suspended_time:
                suspended_epoch = int(str(suspended_time).split('=')[1].rstrip("\\\\n'"))
...
def getPlans():
    ...
    for user in all_users_list:
        ...
        try:
            with open(file_path) as f:
                s = mmap.mmap(f.fileno(), 0, access=mmap.ACCESS_READ)
                position = s.find("PLAN=".encode())
                if position:
                    # We found a line matching PLAN=
                    s.seek(position)
                    line = s.readline()
                    plan = str(line).split("=")[1].strip("\\n'")
                    plans.append(plan)
